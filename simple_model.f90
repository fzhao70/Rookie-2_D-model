...
module io
    ...
    subroutine read(lon, lat, hgt, u, v, nx, ny)
        implicit none
        integer :: nx = 33,ny = 23
        real, dimension(ny, nx) :: lon = 0, lat = 0, hgt = 0
        ...
        ierr = nf_inq_varid(ncid, 'u', latid)
        ierr = nf_inq_varid(ncid, 'v', latid)
        ...
        if (ierr .eq. 0) then
            call message_display("Read File_in Successfully")
        else if
            call message_display("Read File_in Fail")
        end if
...
program simple_model
    ...
    real, dimension(ny, nx) :: Z_next = 0, u_next = 0, v_next = 0
    real, dimension(ny, nx) :: Z_pre = 0, u_pre = 0, v_pre = 0
